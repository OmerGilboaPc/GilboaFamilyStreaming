...
onAuthStateChanged(auth, async (user) => {
    ...
    if (!user) { showOnly("auth"); return; }

    const snap = await get(ref(db, `users/${user.uid}`));
    if (!snap.exists()) await set(ref(db, `users/${user.uid}`), { email: user.email, createdAt: Date.now() });
...
window.enterApp = (profileId) => {
    ...
    els.activeProfileLabel.textContent = state.profiles[profileId]?.name || "פרופיל";
    showOnly("app");
    renderAll();
};
...
function renderAll() {
    ...
    const renderItems = (data, target) => {
        target.innerHTML = Object.entries(data)
            .filter(([id, item]) => item.title.toLowerCase().includes(query))
            .map(([id, item]) => `
                <div class="card" onclick="openDetails('${data === state.movies ? 'movie' : 'series'}', '${id}')">
    ...
    renderItems(state.movies, els.moviesGrid);
    renderItems(state.series, els.seriesGrid);
}
...
function renderRequests() {
    ...
    els.requestsList.innerHTML = list.map(([id, r]) => `
        <div class="request-card" style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:1px solid #333;">
            <div>
                <strong>${esc(r.title)}</strong>
                <div class="small muted">${esc(r.profileName)} • ${new Date(r.createdAt).toLocaleDateString("he-IL")}</div>
            </div>
            ${state.isAdmin ? `<button onclick="deleteRequest('${id}')" style="background:none; border:none; cursor:pointer; font-size:1.2rem;">🗑️</button>` : `<span class="badge">${esc(r.status)}</span>`}
        </div>
...
window.deleteRequest = async (id) => {
    if (confirm("למחוק בקשה זו?")) await remove(ref(db, `requests/${id}`));
};
...
els.signOutBtn.onclick = () => signOut(auth);
els.requestsBtn.onclick = window.showRequests;
if (els.heroRequestsBtn) els.heroRequestsBtn.onclick = window.showRequests;
els.searchInput.oninput = renderAll();

window.randomPick = () => {
    const keys = Object.keys(state.movies);
    if (keys.length) window.openDetails('movie', keys[Math.floor(Math.random() * keys.length)]);
};

if (els.randomBtn) els.randomBtn.onclick = window.randomPick;
